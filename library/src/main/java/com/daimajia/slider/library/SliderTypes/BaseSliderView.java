...
public abstract class BaseSliderView {
  ...
  protected void bindEventAndShow(final View v, ImageView targetImageView) {
    ...
    if (mUrl != null) {
      mGlide = Glide.with(mContext).load(mUrl);
    } else if (mFile != null) {
      mGlide = Glide.with(mContext).load(mFile);
    } else if (mRes != 0) {
      mGlide = Glide.with(mContext).load(mRes);
    } else {
      return;
    }
    switch (mScaleType) {
      case Fit:
        mGlide.apply(RequestOptions.fitCenterTransform());
        break;
      case CenterCrop:
        mGlide.apply(RequestOptions.centerCropTransform());
        break;
      case CenterInside:
        mGlide.apply(RequestOptions.centerInsideTransform());
        break;
    }
    mGlide.listener(new RequestListener<Drawable>() {
      @Override
   ...
   */
  public RequestBuilder<Drawable> getGlide() {
    return mGlide;
  }
   ...
   *              instance
   */
  public void setGlide(RequestBuilder<Drawable> glide) {
    mGlide = glide;
  }
  ...
  public void loadFailed() {
    ...
    if (mView.findViewById(R.id.loading_bar) != null) {
      mView.findViewById(R.id.loading_bar).setVisibility(View.INVISIBLE);
    }
  }
